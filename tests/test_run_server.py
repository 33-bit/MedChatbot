...
def test_kg_needs_build_when_graph_is_empty(monkeypatch):
    monkeypatch.setattr(run_server, "_kg_node_count", lambda: 0)

    assert run_server.kg_needs_build() is True


def test_kg_does_not_need_build_when_graph_has_nodes(monkeypatch):
    monkeypatch.setattr(run_server, "_kg_node_count", lambda: 3)

    assert run_server.kg_needs_build() is False
...
def test_qdrant_build_modes_only_include_missing_or_empty_collections(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda name: {
            run_server.DISEASES_COLLECTION: 0,
            run_server.DRUGS_COLLECTION: 10,
        }[name],
    )

    assert run_server.qdrant_build_modes() == ["--diseases"]


def test_ensure_bootstrap_data_runs_only_needed_builders(monkeypatch):
    commands: list[list[str]] = []
    monkeypatch.setattr(run_server, "kg_needs_build", lambda: True)
    monkeypatch.setattr(run_server, "qdrant_build_modes", lambda: ["--drugs"])
    monkeypatch.setattr(run_server, "_run_module", lambda *args: commands.append(list(args)))
    monkeypatch.setattr(run_server, "ensure_fulltext_indexes", lambda: None)

    run_server.ensure_bootstrap_data()
...
def test_stop_owned_processes_terminates_live_processes_in_reverse_order():
    ...
    run_server.stop_owned_processes([first, second])
...
def test_stop_owned_processes_kills_process_after_timeout():
    ...
    run_server.stop_owned_processes([process])
...
def test_parse_args_accepts_reload():
    args = run_server.parse_args(["--reload"])
...
def test_start_fastapi_omits_reload_by_default(monkeypatch):
    ...
    assert commands == [
        [
            ...
            "run.py",
            "--host",
            run_server.FASTAPI_HOST,
            "--port",
            str(run_server.FASTAPI_PORT),
        ]
    ]
...
def test_start_fastapi_includes_reload_when_requested(monkeypatch):
    ...
    run_server.start_fastapi(reload=True)
...
def test_telegram_webhook_url_strips_trailing_slash():
    assert run_server.telegram_webhook_url("https://demo.ngrok-free.app/") == (
        "https://demo.ngrok-free.app/webhook/telegram"
    )


def test_zalo_webhook_url_strips_trailing_slash():
    assert run_server.zalo_webhook_url("https://demo.ngrok-free.app/") == (
...
def test_register_zalo_webhook_calls_bot_platform_set_webhook(monkeypatch):
    ...
    monkeypatch.setattr(run_server, "ZALO_BOT_TOKEN", "bot-token")
    monkeypatch.setattr(run_server, "ZALO_WEBHOOK_SECRET", "zalo-secret")
    monkeypatch.setattr(run_server.httpx, "post", fake_post)

    run_server.register_zalo_webhook("https://demo.ngrok-free.app/")
...
def test_validate_config_allows_zalo_without_telegram(monkeypatch):
    monkeypatch.setattr(run_server, "REDIS_URL", "redis://localhost:6379/0")
    monkeypatch.setattr(run_server, "NEO4J_URI", "bolt://localhost:7687")
    monkeypatch.setattr(run_server, "NEO4J_PASSWORD", "secret")
    monkeypatch.setattr(run_server, "QDRANT_URL", "http://localhost:6333")
    monkeypatch.setattr(run_server, "NGROK_AUTHTOKEN", "ngrok")
    monkeypatch.setattr(run_server, "TELEGRAM_BOT_TOKEN", "")
    monkeypatch.setattr(run_server, "TELEGRAM_WEBHOOK_SECRET", "")
    monkeypatch.setattr(run_server, "ZALO_BOT_TOKEN", "zalo-token")
    monkeypatch.setattr(run_server, "ZALO_WEBHOOK_SECRET", "zalo-secret")
...
def test_validate_config_requires_a_messaging_channel(monkeypatch):
    ...
    monkeypatch.setattr(run_server, "TELEGRAM_BOT_TOKEN", "")
    monkeypatch.setattr(run_server, "TELEGRAM_WEBHOOK_SECRET", "")
    monkeypatch.setattr(run_server, "ZALO_BOT_TOKEN", "")
    monkeypatch.setattr(run_server, "ZALO_WEBHOOK_SECRET", "")
...
def test_start_owned_process_tracks_process():
    ...
    run_server.start_owned_process(lambda: process, owned)
